...
export class RulesGame {
  ...
  startGame(player1: ApiResponse, player2: ApiResponse, cardTiebreaker: Card[]) {
  ...
  initialSituation(card: Card[], player: string) {
    ...
    if (this.cardTiebreaker[0].suit === card[0].suit) {
      ...
      this.movePlayer[player].points =  this.difference(this.checkValue(this.cardTiebreaker[0].value.toString()), this.checkValue(card[0].value.toString()))
      return 'ta' 
    }
